...
public class Texture : IDisposable
{
    ...
    /// </summary>
    public static bool DefaultSmooth { get; set; } = false;
    ...
    {
        texture = useCache ? Textures.Load(source) : new SFML.Graphics.Texture(source);
        Source = source;
    ...
    {
        texture = useCache ? Textures.Load(stream) : new SFML.Graphics.Texture(stream);
        Source = "stream";
    ...
    /// </summary>
    public Rectangle Region => new(0, 0, Width, Height);

    /// <summary>
    /// Determines if the source texture is smoothed when transformed.
    /// </summary>
    public bool Smooth { get => texture.Smooth; set => texture.Smooth = value; }
    ...
    internal SFML.Graphics.Texture SFMLTexture => texture;
    ...
    public Color GetPixel(int x, int y)
    {
        if (x < 0)
        {
            throw new ArgumentException("X must be greater or equal to than 0.");
        }

        if (y < 0)
        {
            throw new ArgumentException("Y must be greater or equal to than 0.");
        }
