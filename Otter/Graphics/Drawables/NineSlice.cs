...
public class NineSlice : Graphic
{
    ...
    private int
        tWidth,
        tHeight;
    private static readonly Dictionary<string, Rectangle> fillRects = [];
    private int sliceX1, sliceX2, sliceY1, sliceY2;
    private PanelType paneltype;
    private Rectangle panelClip;
    private float panelScaleX, panelScaleY;
    ...
    /// </summary>
    public bool UsePanelClip { set; get; }
    ...
    public int PanelWidth
    {
        set
        {
            ...
            Width = SnapWidth ? GetSnapWidth(value) : value;
            if (Width < 0)
            {
                Width = 0;
            }
        ...
        get => PanelSizeMode == PanelSizeMode.Inside ? Width - sliceX1 + tWidth - sliceX2 : Width;
    }
    ...
    public int PanelHeight
    {
        set
        {
            ...
            Height = SnapHeight ? GetSnapHeight(value) : value;
            if (Height < 0)
            {
                Height = 0;
            }
        ...
        get => PanelSizeMode == PanelSizeMode.Inside ? Height - sliceY1 + tHeight - sliceY2 : Height;
    }
    ...
    private void Initialize(string source, int width, int height, Rectangle? fillRect)
    {
        ...
        if (fillRect == null)
        {
            if (fillRects.TryGetValue(source, out Rectangle rect))
            {
    ...
    private void DrawQuad(VertexArray v, float x1, float y1, float x2, float y2, float u1, float v1, float u2, float v2)
    {
    ...
    protected override void UpdateDrawable()
    {
        ...
        var tileX = sliceX2 - sliceX1;
        var tileY = sliceY2 - sliceY1;
        ...
        {
            for (var xx = x1; xx < x2; xx += tileX)
            {
                for (var yy = y1; yy < y2; yy += tileY)
                {
                    //middle
    ...
    private int GetSnapWidth(int width)
    {
        var sliceWidth = sliceX2 - sliceX1;
        var snapWidth = Width - sliceWidth;
    ...
    private int GetSnapHeight(int height)
    {
        var sliceHeight = sliceY2 - sliceY1;
        var snapHeight = Width - sliceHeight;
