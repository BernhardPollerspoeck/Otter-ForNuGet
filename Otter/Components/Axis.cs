...
public class Axis : Component
{
    ...
    /// The Keys to uss.
    /// </summary>
    public Dictionary<Direction, List<Key>> Keys = [];

    /// <summary>
    /// The joystick buttons to use.
    /// </summary>
    public Dictionary<Direction, List<List<int>>> JoyButtons = [];

    /// <summary>
    /// The X axes to use.
    /// </summary>
    public List<List<JoyAxis>> AxesX = [];

    /// <summary>
    /// The Y axes to use.
    /// </summary>
    public List<List<JoyAxis>> AxesY = [];
    ...
    /// </summary>
    public bool HasInput => ForcedInput || Keys.Count > 0 || JoyButtons.Count > 0 || AxesX.Count > 0 || AxesY.Count > 0;
    ...
    public override void UpdateFirst()
    {
        ...
        for (var i = 0; i < Joystick.Count; i++)
        {
            ...
            {
                var a = Input.GetAxis(axis, i) * 0.01f;
            }
        ...
        for (var i = 0; i < Joystick.Count; i++)
        {
            foreach (var b in JoyButtons[Direction.Up][i])
            {
            ...
            foreach (var b in JoyButtons[Direction.Down][i])
            {
                if (Input.Instance.ButtonDown(b, i))
                {
                    Y += 1;
            ...
            foreach (var b in JoyButtons[Direction.Left][i])
            {
                if (Input.Instance.ButtonDown(b, i))
                {
                    X -= 1;
            ...
            foreach (var b in JoyButtons[Direction.Right][i])
            {
                if (Input.Instance.ButtonDown(b, i))
                {
                    X += 1;
