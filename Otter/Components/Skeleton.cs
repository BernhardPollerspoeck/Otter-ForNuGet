...
public class Skeleton : Component
{
    ...
    private readonly Dictionary<string, Bone> bonesByString = [];
    private readonly Dictionary<Bone, string> stringByBones = [];
    ...
    {
        bone ??= new Bone();
        if (bonesByString.TryGetValue(name, out Bone value))
        {
            stringByBones.Remove(value);
            bone = value;
    ...
    {
        if (stringByBones.TryGetValue(bone, out var value))
        {
            bonesByString.Remove(value);
    ...
    private void ParseXml(XmlNodeList nodes, string parent = "")
    {
        foreach (XmlNode node in nodes)
        {
            ...
            {
                FlipGraphicY = node.AttributeBool("flipGraphicY", false),
                FlipGraphicX = node.AttributeBool("flipGraphicX", false),
                LocalFlipX = node.AttributeBool("flipX", false),
                LocalFlipY = node.AttributeBool("flipY", false)
            };
