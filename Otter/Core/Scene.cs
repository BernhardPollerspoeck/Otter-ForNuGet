...
public class Scene(int width = 0, int height = 0)
{
    #region Private Fields

    private readonly List<Entity> entitiesToAdd = [];
    private readonly List<Entity> entitiesToRemove = [];
    private readonly List<Entity> entitiesToRemoveNextFrame = [];
    private readonly List<Entity> entitiesToChangeLayer = [];
    private readonly List<Entity> entitiesToChangeOrder = [];
    private readonly List<int> groupsToPause = [];
    private readonly List<int> groupsToUnpause = [];
    private readonly List<int> pausedGroups = [];
    private readonly List<Graphic> graphics = [];
    private readonly SortedDictionary<int, List<Entity>> orders = [];
    private readonly SortedDictionary<int, List<Entity>> layers = [];

    internal Dictionary<int, List<Collider>> Colliders = [];
    ...
    /// </summary>
    public Rectangle CameraBounds = new(0, 0, (int)width, (int)height);
    ...
    public Surface Surface
    {
        get
        {
            return Surfaces is not { Count: > 0 } ? null : Surfaces[^1];
        }
        set
        {
            Surfaces.Clear();
    ...
    public Vector2 Center => new(HalfWidth, HalfHeight);
    ...
    /// </summary>
    public float CameraCenterX => cameraX + Game.HalfWidth;

    /// <summary>
    /// The Y position of the center of the camera.
    /// </summary>
    public float CameraCenterY => cameraY + Game.HalfHeight;
    ...
    /// </summary>
    public float CameraWidth => Game.Width / CameraZoom;

    /// <summary>
    /// The height in pixels that the camera is showing with the current zoom.
    /// </summary>
    public float CameraHeight => Game.Height / CameraZoom;
    ...
    /// </summary>
    public Debugger Debugger => Game.Debugger;
    ...
    public Entity this[int id]
    {
        get
        {
            return entitiesById.TryGetValue(id, out Entity value) ? value : null;
        }
    }
    ...
    {
        return GetEntity<T>() switch
        {
            null => !entitiesToAdd.Any(en => en is T) ? Add(e) : (T)entitiesToAdd.Find(en => en is T),
            _ => GetEntity<T>()
        };
    }
    ...
    {
        if (!layers.TryGetValue(e.Layer, out List<Entity> value))
        {
            return;
        }
    ...
    public void SendBackward(Entity e)
    {
        ...
        value.Remove(e);
        value.InsertOrAdd(oldIndex - 1, e);
    }
    ...
    public void UpdateLists()
    {
        while (entitiesToAdd.Count > 0)
        {
            ...
            foreach (var e in adding)
            {
                ...
                if (!orders.TryGetValue(e.Order, out List<Entity> value))
                {
                    value = ([]);
                    orders.Add(e.Order, value);
                }

                value.Add(e);

                if (!layers.TryGetValue(e.Layer, out List<Entity> value2))
                {
                    value2 = ([]);
                    layers.Add(e.Layer, value2);
                }
    ...
    {
        if (!groupsToUnpause.Remove(group))
        {
    ...
    {
        return groupsToPause.Contains(group) || pausedGroups.Contains(group);
    }
    ...
    {
        return layers.TryGetValue(layer, out List<Entity> value) ? value : ([]);
    }
    ...
    internal void AddColliderInternal(Collider c)
    {
        foreach (var tag in c.Tags)
        {
            ...
            {
                if (!Colliders[tag].Contains(c))
                { // Quick fix to prevent double adding.
                    Colliders[tag].Add(c);
                }
            }
            else
    ...
    internal void RemoveColliderInternal(Collider c)
    {
        foreach (var tag in c.Tags)
        {
            if (Colliders.TryGetValue(tag, out List<Collider> value))
            {
                value.Remove(c);
