...
public class PlatformingMovement : Movement
{
    ...
    private int ledgeBuffer = 0;
    private readonly List<Speed> Speeds = [];
    ...
    /// </summary>
    public Dictionary<AccelType, int> Acceleration = [];

    /// <summary>
    /// The default acceleration value to use if none are set.
    /// </summary>
    private static readonly int DefaultAccleration = 150;
    ...
    public override void Update()
    {
        ...
        if (JumpEnabled)
        {
            ...
            {
                if (DownJumpDrop && JumpThroughCollider != null)
                { // Drop through platforms with Down + Jump
                    var jumping = true;
                    if (Axis.Y > 0.5f)
                    {
        ...
        if (!OnGround && !HasJumped && ledgeBuffer == 0)
        {
            JumpsLeft = JumpsMax - 1;
        }

        CurrentAccel = OnGround ? Acceleration[AccelType.Ground] : Acceleration[AccelType.Air];
    ...
    public override void MoveY(int speed, Collider collider = null)
    {
        ...
        {
            var move = Math.Sign(MoveBufferY);
            if (collider != null)
            {
                var freeToMove = true;
                Collider c;
