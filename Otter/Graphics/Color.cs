...
public class Color
{
    ...
    public static Color Lerp(float amount, params Color[] colors)
    {
        ...
        var fromIndex = (int)Util.ScaleClamp(amount, 0, 1, 0, colors.Length - 1);
        var toIndex = fromIndex + 1;

        var length = 1f / (colors.Length - 1);
        var lerp = Util.ScaleClamp(amount % length, 0, length, 0, 1);
    ...
    public static Color Orange => new(1f, 0.5f, 0);

    public static Color Gold => new("FFCC00");

    public static Color None => new(0f, 0f, 0f, 0f);

    public static Color Grey => new("999999");

    public static Color Gray => Grey;

    public static Color Random => Rand.Color;

    public static Color RandomAlpha => Rand.ColorAlpha;

    #endregion

    #region Static Fields

    private static readonly Dictionary<string, Color> customColors = [];
    ...
    public Color(string hex)
    {
        ...
        {
            red = Convert.ToInt32(hex[..1] + hex[..1], 16);
            green = Convert.ToInt32(string.Concat(hex.AsSpan(1, 1), hex.AsSpan(1, 1)), 16);
            blue = Convert.ToInt32(string.Concat(hex.AsSpan(2, 1), hex.AsSpan(2, 1)), 16);
        }
        ...
        else if (hex.Length == 4)
        {
            ...
            green = Convert.ToInt32(string.Concat(hex.AsSpan(1, 1), hex.AsSpan(1, 1)), 16);
            blue = Convert.ToInt32(string.Concat(hex.AsSpan(2, 1), hex.AsSpan(2, 1)), 16);
            alpha = Convert.ToInt32(string.Concat(hex.AsSpan(3, 1), hex.AsSpan(2, 1)), 16);
        }
    ...
    /// </summary>
    public string ColorString => ByteR.ToString("X2") + ByteG.ToString("X2") + ByteB.ToString("X2") + ByteA.ToString("X2");
    ...
    internal SFML.Graphics.Color SFMLColor => new(ByteR, ByteG, ByteB, ByteA);
