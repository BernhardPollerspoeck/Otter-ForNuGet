...
public static class Util
{
    ...
    public static float LerpSet(float amount, params float[] numbers)
    {
        ...
        var fromIndex = (int)ScaleClamp(amount, 0, 1, 0, numbers.Length - 1);
        var toIndex = fromIndex + 1;

        var length = 1f / (numbers.Length - 1);
        var lerp = ScaleClamp(amount % length, 0, length, 0, 1);
    ...
    public static Color LerpColor(float amount, params Color[] colors)
    {
        ...
        var fromIndex = (int)ScaleClamp(amount, 0, 1, 0, colors.Length - 1);
        var toIndex = fromIndex + 1;

        var length = 1f / (colors.Length - 1);
        var lerp = ScaleClamp(amount % length, 0, length, 0, 1);
    ...
    {
        return width == 0 ? new Vector2(index, 0) : new Vector2(index % width, index / width);
    }
    ...
    {
        return src == null || !HasField(src, fieldName) ? returnOnNull : GetFieldValue(src, fieldName);
    }
...
#pragma warning restore IDE0046 // Convert to conditional expression
    ...
    public static Dictionary<string, string> StringToDictionary(string source, string keydelim, string valuedelim)
    {
        ...
        var split = Regex.Split(source, valuedelim);
    ...
    {
        if (enumStringCache.TryGetValue(enumValue, out var value))
        {
            return value;
        }

        var str = string.Format("{0}.{1}", enumValue.GetType(), enumValue);
        enumStringCache.Add(enumValue, str);
        return enumStringCache[enumValue];
    }

    private static readonly Dictionary<Enum, string> enumStringCache = [];
    ...
    /// </summary>
    public static int DesktopWidth => (int)SFML.Window.VideoMode.DesktopMode.Width;

    /// <summary>
    /// The height of the desktop.  Note that this is for the display that the game was initialized in.
    /// </summary>
    public static int DesktopHeight => (int)SFML.Window.VideoMode.DesktopMode.Height;
