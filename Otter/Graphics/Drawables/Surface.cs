...
public class Surface : Image
{
    ...
    private float cameraX, cameraY, cameraAngle, cameraZoom = 1f;
    ...
    private string saveNameFramePath;
    ...
    public float CameraWidth => Width / CameraZoom;

    public float CameraHeight => Height / CameraZoom;

    /// <summary>
    /// The Texture the Surface has rendered to.
    /// </summary>
    public override Texture Texture => new(renderTexture.Texture);
    ...
    {
        View v = new(new FloatRect(cameraX, cameraY, Width, Height))
        {
    ...
    public void DrawToWindow(Game game)
    {
        ...
        {
            saveNextFrame = false;
            SFML.Graphics.Texture texture = new(game.Window.Size.X, game.Window.Size.Y);
            texture.Update(game.Window);
            SFML.Graphics.Image img = texture.CopyToImage();
            img.SaveToFile(saveNameFramePath);
        }
    ...
    internal void Draw(float x, float y, int width, int height, Color color = null, BlendMode blend = BlendMode.Alpha)
    {
        states = new RenderStates(Texture.SFMLTexture)
        {
            //states.BlendMode = (SFML.Graphics.BlendMode)Blend;
            BlendMode = SFMLBlendMode(blend)
        };
    ...
    internal RenderTarget RenderTarget => renderTexture;
    ...
    /// </summary>
    /// <returns></returns>
    private Drawable RenderShaders()
    {
        ...
        else
        {
            ...
            else if (shaders.Count > 2)
            {
                ...
                for (var i = 1; i < shaders.Count - 1; i++)
                {
