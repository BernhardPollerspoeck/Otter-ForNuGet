...
public class TileInfo
{
    ...
    internal Color tilemapColor = new();

    internal Vector2f SFMLPosition => new(X, Y);

    internal Vector2f SFMLTextureCoord => new(TX, TY);
    ...
    internal Vertex CreateVertex(int x = 0, int y = 0, int tx = 0, int ty = 0)
    {
        ...
        return TX == -1 || TY == -1
            ? new Vertex(new Vector2f(X + x, Y + y), tileColor.SFMLColor)
            : new Vertex(new Vector2f(X + x, Y + y), tileColor.SFMLColor, new Vector2f(TX + tx, TY + ty));
    }
