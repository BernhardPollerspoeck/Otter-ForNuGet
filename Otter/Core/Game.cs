...
#pragma warning restore IDE0079 // Remove unnecessary suppression
    ...
    private readonly Stopwatch renderTime = new();
    private readonly Stopwatch updateTime = new();
    private readonly Stopwatch gameTime = new();
    private float cameraAngle, cameraZoom;
    ...
    private readonly List<Scene> goToScenes = [];
    private Scene goToScene;
    private List<Scene> scenesToRender = [];
    private int removeSceneCount = 0;
    private string gameFolder;
    private readonly List<float> fpsTimes = [];
    private int nextFpsIndex = 0;
    private readonly int fpsLogSize = 20;
    private bool windowSet = false;
    private bool updatedOnce = false;
    private uint iconWidth, iconHeight;
    private SFML.Graphics.Image iconImage;
    ...
    /// </summary>
    public Button ScreenshotButton = new();
    ...
    /// </summary>
    public string Filepath => FileHandling.GetAbsoluteFilePath(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "/" + GameFolder + "/");
    ...
    /// </summary>
    public int Second => (int)TargetFramerate;
    ...
    public float CameraZoom
    {
        ...
        set
        {
            ...
            if (cameraZoom <= 0)
            {
                cameraZoom = 0.0001f; //no divide by 0 please
            }
    ...
    /// </summary>
    public bool MouseVisible { set; get; }

    /// <summary>
    /// The amount of time in milliseconds for the render pass to complete.
    /// </summary>
    public int RenderTime => (int)renderTime.ElapsedMilliseconds;

    /// <summary>
    /// The amount of time in milliseconds for the update time to complete.
    /// </summary>
    public int UpdateTime => (int)updateTime.ElapsedMilliseconds;
    ...
    public Game(string title = "Game", int width = 640, int height = 480, int targetFramerate = 60, bool fullscreen = false)
    {
        ...
        {
            ExportMode = DataSaver.DataExportMode.Config
        };
    ...
    private void ResetWindow(string title, int width, int height, bool fullscreen, bool vsync)
    {
        if (width < 0)
        {
...
#pragma warning restore IDE0079 // Remove unnecessary suppression
    ...
    private void UpdateScenes()
    {
        if (goToScene == null)
        {
            ...
            {
                for (var i = 0; i < removeSceneCount; i++)
                {
            ...
            {
                for (var i = 0; i < goToScenes.Count; i++)
                {
                    Scene?.PauseInternal();
    ...
    private void Render()
    {
        ...
        if (DrawInactiveScenes)
        {
            ...
            for (var i = 0; i < scenesToRender.Count; i++)
            {
    ...
    private void OnResize(object send, SizeEventArgs e)
    {
    ...
    public Scene Scene
    {
        get => Scenes.Count == 0 ? null : Scenes.Peek();
        private set { }
    }
