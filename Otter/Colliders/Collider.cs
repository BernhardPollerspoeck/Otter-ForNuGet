...
public abstract class Collider
{
    ...
    public bool Overlap(float x, float y, int px, int py)
    {
        ...
        var _ = new Entity(0, 0, null, point);
    ...
    public Collider Collide(float x, float y, params int[] tags)
    {
        ...
        if (tags.Length == 0)
        {
            ...
            var id = 0;
            foreach (var i in Entity.Scene.Colliders.Keys)
            {
                tags[id] = i;
                id++;
            }
        ...
        {
            if (Entity.Scene.Colliders.TryGetValue(t, out List<Collider> value))
            {
                foreach (var c in value)
                {
                    if (c.Entity != null)
                    {
                        if (!c.Entity.Collidable)
                        {
                            continue;
                        }
    ...
    public List<Entity> CollideEntities(float x, float y, List<int> tags)
    {
        ...
        foreach (var tag in tags)
        {
            ...
            foreach (Collider col in c)
            {
                if (collided.Contains(col.Entity))
                {
                    continue;
                }
    ...
    internal static bool OverlapTest(Collider first, Collider second)
    {
        ...
        else if ((first is BoxCollider && second is CircleCollider) || (first is CircleCollider && second is BoxCollider))
        {
            ...
            if (boxLine.IntersectCircle(new Vector2(circle.CenterX, circle.CenterY), circle.Radius))
            {
                return true;
            }
            ...
            return boxLine.IntersectCircle(new Vector2(circle.CenterX, circle.CenterY), circle.Radius);
        }
        #endregion
        ...
        else if ((first is BoxCollider && second is GridCollider) || (first is GridCollider && second is BoxCollider))
        {
            ...
            if (box.Right <= grid.Left)
            {
                return false;
            }

            if (box.Bottom <= grid.Top)
            {
                return false;
            }

            if (box.Left >= grid.Right)
            {
                return false;
            }

            if (box.Top >= grid.Bottom)
            {
                return false;
            }

            //This is a quick fix and might have to be addressed later
            return grid.GetRect(box.Left, box.Top, box.Right - 1, box.Bottom - 1, false);
        }
        #endregion
        ...
        else if ((first is BoxCollider && second is LineCollider) || (first is LineCollider && second is BoxCollider))
        {
            ...
            return line.Line2.IntersectsRect(box.Left, box.Top, box.Width, box.Height);
        }
        #endregion
        ...
        else if (first is CircleCollider && second is CircleCollider)
        {
            ...
            return Util.Distance(circle1.CenterX, circle1.CenterY, circle2.CenterX, circle2.CenterY) < circle1.Radius + circle2.Radius;
        }
        #endregion
        ...
        else if ((first is CircleCollider && second is GridCollider) || (first is GridCollider && second is CircleCollider))
        {
            ...
            {
                float rectX, rectY;
                for (var i = gridx; i <= gridx2; i++)
                {
                    for (var j = gridy; j <= gridy2; j++)
                    {
                        if (grid.GetTile(i, j))
                        {
        ...
        else if ((first is LineCollider && second is CircleCollider) || (first is CircleCollider && second is LineCollider))
        {
            ...
            return line.Line2.IntersectCircle(new Vector2(circle.CenterX, circle.CenterY), circle.Radius);
        }
        #endregion
        ...
        else if (first is GridCollider && second is GridCollider)
        {
            ...
            for (var i = 0; i < small.TileColumns; i++)
            {
                for (var j = 0; j < small.TileRows; j++)
                {
        ...
        else if ((first is PointCollider && second is CircleCollider) || (first is CircleCollider && second is PointCollider))
        {
            ...
            return Util.Distance(point.Left, point.Top, circle.CenterX, circle.CenterY) < circle.Radius;
        }
        #endregion
        ...
        else if ((first is PointCollider && second is BoxCollider) || (first is BoxCollider && second is PointCollider))
        {
            ...
            return Util.InRect(point.Left, point.Top, box.Left, box.Top, box.Width, box.Height);
        }
        #endregion
        ...
        else if ((first is PointCollider && second is LineCollider) || (first is LineCollider && second is PointCollider))
        {
            ...
            return (point.Left - line2.X1) / (line2.X2 - line2.X1) == (point.Top - line2.Y1) / (line2.Y2 - line2.Y1);
        }
        #endregion
        ...
        else if ((first is PixelCollider && second is BoxCollider) || (first is BoxCollider && second is PixelCollider))
        {
            ...
            if (box.Right <= pixel.Left)
            {
                return false;
            }

            if (box.Left >= pixel.Right)
            {
                return false;
            }

            if (box.Top >= pixel.Bottom)
            {
                return false;
            }

            if (box.Bottom <= pixel.Top)
            {
                return false;
            }
            ...
            var x1 = Math.Max(box.Left, pixel.Left);
            var x2 = Math.Min(box.Right, pixel.Right);

            var y1 = Math.Max(box.Top, pixel.Top);
            var y2 = Math.Min(box.Bottom, pixel.Bottom);
        ...
        else if ((first is PixelCollider && second is PointCollider) || (first is PointCollider && second is PixelCollider))
        {
            ...
            if (!Util.InRect(point.Left, point.Top, pixel.Left, pixel.Top, pixel.Width, pixel.Height))
            {
                return false;
            }

            //check for pixel at point
            return pixel.PixelAtRelative((int)point.Left, (int)point.Top);
        }
        #endregion
        ...
        else if ((first is PixelCollider && second is LineCollider) || (first is LineCollider && second is PixelCollider))
        {
            ...
            if (!line.Line2.IntersectsRect(pixel.Left, pixel.Top, pixel.Width, pixel.Height))
            {
                return false;
            }
        ...
        else if ((first is PixelCollider && second is GridCollider) || (first is GridCollider && second is PixelCollider))
        {
            ...
            if (pixel.Right <= grid.Left)
            {
                return false;
            }

            if (pixel.Bottom <= grid.Top)
            {
                return false;
            }

            if (pixel.Left >= grid.Right)
            {
                return false;
            }

            if (pixel.Top >= grid.Bottom)
            {
                return false;
            }

            //This is a quick fix and might have to be addressed later
            if (!grid.GetRect(pixel.Left, pixel.Top, pixel.Right - 1, pixel.Bottom - 1, false))
            {
                return false;
            }
            ...
            for (var xx = 0; xx < pixel.Width; xx++)
            {
                for (var yy = 0; yy < pixel.Height; yy++)
                {
                    var checkx = xx + pixel.Left;
                    var checky = yy + pixel.Top;
        ...
        else if ((first is PolygonCollider && second is GridCollider) || (first is GridCollider && second is PolygonCollider))
        {
            ...
            var startX = grid.GridX(poly.Left - grid.Left);
            var startY = grid.GridY(poly.Top - grid.Top);

            var endX = grid.GridX(poly.Left - grid.Left + poly.Width) + 1;
            var endY = grid.GridY(poly.Top - grid.Top + poly.Height) + 1;
        ...
        else if ((first is PolygonCollider && second is PixelCollider) || (first is PixelCollider && second is PolygonCollider))
        {
            ...
            var x1 = Math.Max(poly.Left, pixel.Left);
            var x2 = Math.Min(poly.Right, pixel.Right);

            var y1 = Math.Max(poly.Top, pixel.Top);
            var y2 = Math.Min(poly.Bottom, pixel.Bottom);
