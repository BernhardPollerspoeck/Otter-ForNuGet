...
public class GridCollider : Collider
{
    ...
    private readonly List<bool> collisions = [];
    ...
    public GridCollider(int width, int height, int tileWidth, int tileHeight, params int[] tags)
    {
        ...
        for (var i = 0; i < TileRows * TileColumns; i++)
        {
    ...
    public bool GetTile(int x, int y)
    {
        ...
        return index < collisions.Count && collisions[index];
    }
    ...
    public bool GetRect(float x, float y, float x2, float y2, bool usingGrid = true)
    {
        ...
        for (var i = (int)x; i <= x2; i++)
        {
            for (var j = (int)y; j <= y2; j++)
            {
                if (GetTile(i, j))
                {
                    return true;
    ...
    public void LoadTilemap(Tilemap source, int layerDepth)
    {
        ...
        {
            throw new ArgumentException("Tilemap size and tile size must match grid size and tile size.");
        }
    ...
    public override void Render(Color color = null)
    {
        ...
        var viewLeft = Game.Instance.Scene.CameraX - TileWidth;
        var viewRight = Game.Instance.Scene.CameraX + Game.Instance.Scene.CameraWidth;
        var viewTop = Game.Instance.Scene.CameraY - TileHeight;
        var viewBottom = Game.Instance.Scene.CameraY + Game.Instance.Scene.CameraHeight;
